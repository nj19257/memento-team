...
# Options: --engine, --country, --lang, --location, --json, --api-key
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      echo "Usage: search.sh <query> [--engine google|google_news|google_local] [--country br] [--lang pt] [--location 'São Paulo'] [--json]"
      exit 0
      ;;
...
BASE_PARAMS="q=${Q}&api_key=${API_KEY}&gl=${COUNTRY}&hl=${LANG}"
[[ -n "$LOCATION" ]] && BASE_PARAMS="${BASE_PARAMS}&location=$(urlencode "$LOCATION")"

case "$ENGINE" in
  google)
    PARAMS="engine=google&${BASE_PARAMS}"
    ;;
  google_news)
    PARAMS="engine=google_news&${BASE_PARAMS}"
    ;;
  google_local)
    PARAMS="engine=google_local&${BASE_PARAMS}"
    ;;
  *)
