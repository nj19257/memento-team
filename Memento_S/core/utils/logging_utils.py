...
def log_event(event: str, **fields: Any) -> None:
    ...
    The event is always appended to the active trajectory (thread-local
    and async ContextVar) regardless of the ``EXEC_LOG_ENABLED`` flag,
    so that trajectory collection works even when file logging is off.
    """
    record: dict[str, Any] = {
    ...
    # Append to per-thread trajectory if active
    traj_events = getattr(_TRAJECTORY_STORE, "events", None)
    if traj_events is not None:
        traj_events.append(record)

    # Append to per-asyncio-task trajectory if active
    ctx_events = _TRAJECTORY_CTX_EVENTS.get()
    if ctx_events is not None:
        ctx_events.append(record)

    # File logging (gated by EXEC_LOG_ENABLED)
...
# ---------------------------------------------------------------------------
# Per-thread trajectory collection
# ---------------------------------------------------------------------------
_TRAJECTORY_STORE: threading.local = threading.local()


def start_trajectory(label: str = "") -> None:
    """Begin collecting trajectory events for the current thread."""
    _TRAJECTORY_STORE.events = []
    _TRAJECTORY_STORE.label = label


def collect_trajectory() -> list[dict]:
    """Return collected trajectory events and reset for the current thread."""
    events = getattr(_TRAJECTORY_STORE, "events", [])
    _TRAJECTORY_STORE.events = []
    return events


# ---------------------------------------------------------------------------
# Per-asyncio-task trajectory collection (ContextVar-based)
# ---------------------------------------------------------------------------
_TRAJECTORY_CTX_EVENTS: ContextVar[list[dict] | None] = ContextVar(
    "_trajectory_ctx_events", default=None
)


def start_trajectory_async(label: str = "") -> None:
    """Begin collecting trajectory events for the current asyncio task.

    Each ``asyncio.Task`` inherits its own copy of the ContextVar,
    so workers running concurrently get independent trajectories.
    """
    _TRAJECTORY_CTX_EVENTS.set([])


def collect_trajectory_async() -> list[dict]:
    """Return collected trajectory events and reset for the current asyncio task."""
    events = _TRAJECTORY_CTX_EVENTS.get() or []
    _TRAJECTORY_CTX_EVENTS.set(None)
    return events
