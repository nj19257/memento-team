...
def _resolve_path(raw: str) -> Path:
    ...
    if not p.is_absolute():
        return _base_dir / p
    # Already absolute – use it directly only if it already exists or has
    # any existing ancestor beyond the root (e.g. inside /tmp, user home).
    # Otherwise assume the LLM meant it relative to _base_dir.
    check = p
    while check != check.parent:  # walk up until root
        if check.exists():
            return p
        check = check.parent
