...
These apply to ALL task types — read the matched `decompose-<type>` skill for type-specific guidance.
...
acquire new skills on demand. Focus on decomposing the task into clear, self-contained subtasks.

## How to Use
1. Read the user's query
2. Match it against the task types below
3. Call `read_skill("decompose-<matched_type>")` to load the specialized strategy
...
- When asked for a comprehensive list (e.g. "all X in category Y"), first search for the list of relevant elements, then create separate subtasks for each element to find details